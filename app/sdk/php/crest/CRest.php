...
class CRest
{
	private BitrixIntegration $BitrixIntegration;

	const VERSION = '1.36';
	const BATCH_COUNT    = 50;//count batch 1 query
	const TYPE_TRANSPORT = 'json';// json or xml

	public function __construct(BitrixIntegration $BitrixIntegration)
	{
		$this->BitrixIntegration = $BitrixIntegration;
	}
	...
	public function GetNewTokens()
	{
		$queryParams = [
			...
			'refresh_token' => $this->BitrixIntegration->GetRefreshToken(),
		];
		$link = "https://oauth.bitrix.info/oauth/token/?" . http_build_query($queryParams);
		$curl = curl_init($link);
		...
		$this->BitrixIntegration->SetAccessToken($response['access_token']);
		$this->BitrixIntegration->SetRefreshToken($response['refresh_token']);
		$this->BitrixIntegration->Save();
	}
	...
	protected function callCurl($arParams)
	{
		...
		$arSettings = $this->getAppSettings($arParams['bitrix_url']);
	...
	public function call($method, $params = [])
	{
		$arPost = [
			'method' 	 => $method,
			'params' 	 => $params,
			'bitrix_url' => $this->BitrixIntegration->GetProfileURL()
		];
	...
	{
		$result = [];
		$arSettings = $this->getAppSettings(null);
		if($arSettings !== false)
		{
			$arParamsAuth = [
				'this_auth' => 'Y',
	...
	private function setAppSettings($arSettings, $isInstall = false)
	{
		...
		{
			$oldData = $this->getAppSettings(null);
	...
	private function getAppSettings($bitrix_url)
	{
			$arData = $this->getSettingData($bitrix_url);				
			$isCurrData = false;
	...
	protected function getSettingData($bitrix_url)
	{
		$return['access_token'] 			= $this->BitrixIntegration->GetAccessToken();
		$return['domain'] 					= $this->BitrixIntegration->GetProfileURL();
		$return['refresh_token'] 			= $this->BitrixIntegration->GetRefreshToken();
		$return['application_token'] 		= $this->BitrixIntegration->GetApplicationToken();
		$return['client_endpoint'] 			= 'https://' . $this->BitrixIntegration->GetProfileURL() . '/rest/';
	...
	{
		$this->BitrixIntegration->SetProfileURL($arSettings['domain']);
		$this->BitrixIntegration->SetAccessToken($arSettings['access_token']);
		$this->BitrixIntegration->SetRefreshToken($arSettings['refresh_token']);
		$this->BitrixIntegration->SetApplicationToken($arSettings['application_token']);
		$this->BitrixIntegration->Save();
